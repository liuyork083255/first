...
public class GlobalExceptionController {
    ...
    /**
     * 捕获自定义 NcdExceptionModel 异常
     *
     * @param e
     * @return
     */
    @ResponseStatus(HttpStatus.OK)
    @ExceptionHandler(value = NcdExceptionModel.class)
    public ResponseData cacheNcdException(NcdExceptionModel e, HttpServletRequest request) {
        logger.error("程序自定义异常捕获.    exception:{}", e.getMessage());
        return ResponseUtil.error(e.getCode(), e.getMessage(), request);
    }

    /**
     * 捕获自定义 NcdWarnException 异常
     *
     * @param e
     * @return
     */
    @ExceptionHandler(value = NcdWarnException.class)
    public NullDTO cacheNcdWarnException(NcdWarnException e) {
        logger.warn("程序自定义警告捕获.    warn:{}", e.getMessage());
        return ResponseUtil.nullDTO(e.getMessage());
    }
